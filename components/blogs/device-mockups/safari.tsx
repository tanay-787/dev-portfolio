...
// ─── Frame geometry ──────────────────────────────────────────────────────────
// SH is derived so the content area ratio = exactly 9:16 (1916×1078 ≈ 1.7774:1)
// meaning a true 1920×1080 image fills the screen with < 0.25 px height crop.
// The overall component aspect ratio becomes 1920:1154 (~1.663:1).
const W  = 1920                        // total width
const CH = 74                          // chrome toolbar height
const CR = 16                          // outer corner radius
...
const SY = CH                          //   74
const SW = W - 2 * B                   // 1916
const SH = Math.round(SW * 9 / 16)    // 1078  ← 16:9 content
const H  = SY + SH + B                // 1154  ← total height

// Percentages for the HTML media overlay div
const LEFT_PCT   = (SX / W) * 100   // ≈ 0.104
const TOP_PCT    = (SY / H) * 100   // ≈ 6.852
const WIDTH_PCT  = (SW / W) * 100   // ≈ 99.792
const HEIGHT_PCT = (SH / H) * 100   // ≈ 92.963

// ─── Scale factors to reuse all original 1203 × 753 SVG paths ───────────────
// Every original element is wrapped in <g transform={ORIG_SCALE}> so the
// traffic-lights, URL bar, lock icon, and toolbar icons land in the right
// place without manually recomputing hundreds of path coordinates.
const ORIG_SCALE = `scale(${(W / 1203).toFixed(6)}, ${(H / 753).toFixed(6)})`
// ≈ scale(1.596011, 1.434262)

// ─── Pre-computed SVG paths (bezier quarter-circle: k = r * 0.4477) ─────────

// Outer chrome top band – rounded top corners only, fills y 0→CH
const CHROME_PATH =
  `M0 16C0 7.163 7.163 0 16 0H1904C1912.837 0 1920 7.163 1920 16V74H0V16Z`

// Outer screen border – rounded bottom corners only, fills y CH→H
const SCREEN_BORDER_PATH =
  `M0 74H1920V1138C1920 1146.837 1912.837 1154 1904 1154H16C7.163 1154 0 1146.837 0 1138V74Z`

// Inner chrome (white / dark) – 2 px inset, radius 14, fills y B→(CH-B)
const INNER_CHROME_PATH =
  `M2 16C2 8.268 8.268 2 16 2H1904C1911.732 2 1918 8.268 1918 16V72H2V16Z`

// Content-area punch-out for the mask – concentric with outer frame corners
// Corner centers shared with SCREEN_BORDER_PATH: (W−CR, H−CR) and (CR, H−CR)
// Inner radius = CR−B = 14, ensuring a constant 2 px border through the curve
const PUNCH_PATH =
  `M2 74H1918V1138C1918 1145.732 1911.732 1152 1904 1152H16C8.268 1152 2 1145.732 2 1138V74Z`
...
export function Safari({
  ...
  // Unique IDs prevent conflicts when multiple instances are on the same page
  const uid    = useId()
  const maskId = `sfW-mask${uid}`
  const clipId = `sfW-clip${uid}`
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          {/* Punch-out mask: white = visible frame, black = transparent hole */}
          <mask id={maskId} maskUnits="userSpaceOnUse">
            <rect x="0" y="0" width={W} height={H} fill="white" />
            <path d={PUNCH_PATH} fill="black" />
          </mask>

          {/* Clip path keeps everything inside the outer rounded rectangle */}
          <clipPath id={clipId}>
        ...
        >
          {/* ── Outer frame background (gray) ─────────────────────────── */}
          <path d={CHROME_PATH}        className="fill-[#E5E5E5] dark:fill-[#404040]" />
          <path d={SCREEN_BORDER_PATH} className="fill-[#E5E5E5] dark:fill-[#404040]" />

          {/* ── Inner chrome (white / dark) ───────────────────────────── */}
          <path d={INNER_CHROME_PATH} className="fill-white dark:fill-[#262626]" />

          {/* ── Chrome elements scaled from original 1203 × 753 space ─── */}
          {/*
           *  ORIG_SCALE ≈ scale(1.596, 1.434) maps every original coordinate
           *  into the new 1920 × 1080 viewport accurately, so the traffic
           *  lights, URL bar and all icons land in proportionally correct
           *  positions without recomputing hundreds of path points.
           */}
          <g transform={ORIG_SCALE}>
            {/* Traffic lights */}
            ...
            {/* ── mode="default" toolbar icons ──────────────────────── */}
            {mode === "default" && (
              <>
                {/* Privacy / shield icon (left of URL bar) */}
                <g className="mix-blend-luminosity">
                  <path
