/**
 * @library Internal
 * @inspired-from https://magicui.design/docs/components/icon-cloud
 */
...
/* 
  CHANGES: 
   - Icons are now theme aware
   - Fixed icons appearing small
    ...
    const newIconCanvases = items.map((item, index) => {
      ...
      if (offCtx) {
        if (images) {
          ...
          // 👇 inject computed color into SVG
          svgString = svgString.replace(/currentColor/g, foreground);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      iconPositions.forEach((icon, index) => {
        ...
        // Adjust scale and position based on new width/height
        const scaleFactor = Math.min(width, height) / 400; // Original canvas size was 400x400
